...
echo "=== Patch Script: Clone and Patch CloudWatch MCP Server ==="
...
with open("awslabs/cloudwatch_mcp_server/server.py", "r") as f:
    content = f.read()
...
with open("awslabs/cloudwatch_mcp_server/server.py", "w") as f:
    f.write(content)
...
# Validate patch
grep -q 'streamable-http' "awslabs/cloudwatch_mcp_server/server.py" || { echo "ERROR: streamable-http not found in server.py"; exit 1; }
grep -q "mcp.settings.host" "awslabs/cloudwatch_mcp_server/server.py" || { echo "ERROR: host setting not found in server.py"; exit 1; }
grep -q "mcp.settings.port = 8000" "awslabs/cloudwatch_mcp_server/server.py" || { echo "ERROR: port=8000 not found in server.py"; exit 1; }
...
EXPOSE 8000
' Dockerfile
    sed -i '' 's|ENTRYPOINT.*|ENTRYPOINT ["python", "-m", "awslabs.cloudwatch_mcp_server.server"]|' Dockerfile
else
...
echo "=== All CloudWatch MCP server patches complete ==="
